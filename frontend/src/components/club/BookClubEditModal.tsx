...
interface BookClubEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  bookClub: BookClub | null;
  onUpdate: (updatedClub: any) => void;
}

export function BookClubEditModal({
  isOpen,
  onClose,
  bookClub,
  onUpdate,
}: BookClubEditModalProps) {
  ...
  // Set form values when the modal is opened or bookClub changes
  useEffect(() => {
    if (isOpen && bookClub) {
      setName(bookClub.name || "");
      setDescription(bookClub.club_desc || "");
      setAboutContent(bookClub.about_content || "");
      setIsPrivate(bookClub.is_private || false);
      setClubImagePreview(
        bookClub.club_image
          ? `http://localhost:8000${bookClub.club_image}`
          : null
      );

      if (bookClub.book && bookClub.book_title) {
        setSelectedBook({
          book_id: bookClub.book,
          title: bookClub.book_title,
          authors: [], // You might want to fetch the full book details to get authors
        });
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/bookclubs/clubs/${bookClub.id}/update/`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      // If we have a new club image, upload it
      if (clubImage) {
        try {
          ...
          const imageResponse = await fetch(
            `http://localhost:8000/api/bookclubs/clubs/${bookClub.id}/update-image/`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          if (!imageResponse.ok) {
            console.error("Failed to upload club image, but club was updated");
          } else {
            const imageData = await imageResponse.json();
            updatedClub.club_image = imageData.club_image_url;
          }
        } catch (imageErr) {
      ...
      // Call the onUpdate callback with the updated club data
      onUpdate(updatedClub);
      onClose();
    } catch (err) {
      console.error("Error updating book club:", err);
      setError(
        err instanceof Error ? err.message : "Failed to update book club"
      );
    } finally {
      setIsSubmitting(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="relative w-full max-w-2xl rounded-lg bg-amber-50 p-6 shadow-lg">
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label htmlFor="about-content" className="text-amber-800">
              About Content
            </Label>
            <Textarea
              id="about-content"
          ...
          <div>
            <Label className="text-amber-800 mb-2 inline-block">
              Current Book
            </Label>
            {selectedBook ? (
              <div className="mb-4 rounded-md border border-amber-300 bg-amber-50 p-3">
                <div className="flex items-center gap-3">
                  <div className="h-16 w-12 flex-shrink-0 overflow-hidden rounded-sm bg-amber-200">
              ...
              <div className="mb-4">
                <p className="text-sm text-amber-700">No book selected</p>
              </div>
            )}

            <div className="mb-4">
              <Label htmlFor="book-search" className="text-amber-800">
                Change Book (Optional)
              </Label>
